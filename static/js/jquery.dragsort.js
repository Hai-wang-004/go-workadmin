		...
		this.each(function(i, cont) {
			...
			var newList = {
				...
				init: function() {
					...
					$(this.container).children(opts.itemSelector).each(function(j) { $(this).attr("data-itemIdx", j); $(this).find("span").html( (j+1) + "、"); });
				},
				...
				dropItem: function() {
					...
					//$(list.container).find(opts.dragSelector).css("background-color", "#CAC8FB");
					...
					$(lists).each(function() {
						$(this.container).children(opts.itemSelector).each(function(j) {
							...
							//popup.alert($(this).find("span").html());
                                $(this).find("span").html( (j + 1 ) + "、");
							}
						});
					});
					if (changed)
