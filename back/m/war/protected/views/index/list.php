...
<tr <?=$active?>>
    	...
    	<td>
    		<?php 
    		if($auction): 
    			$buyUrl = $this->createUrl( "index/cancel/?auc={$v['auc']}&xsToken={$xsToken}" );
				$scanUrl= $this->createUrl( "index/index/?itemId={$v['itemId']}" ); 
    			?>
    			<a target="_blank" class="btn btn-primary" href="<?=$scanUrl?>">浏览</a>
    			<button class="btn btn-danger" onclick="return wow_cancel(this,'<?=$buyUrl?>','');">取消</button>
    		<?php else: 
    			$buyUrl = $this->createUrl( "index/buy/?auc={$v['auc']}&money={$v['money']}&xsToken={$xsToken}" );
